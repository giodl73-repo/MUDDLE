...
| Browser save slots | Players can save, inspect, copy paths for, load, and delete named sibling save files from the browser while keeping the configured active `--save` path. |
...
- in-window named save-slot create/inspect/copy-path/load/delete controls over sibling command-replay save files
