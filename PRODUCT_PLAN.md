...
- `muddle-macroquad` lightweight game-window client with host chooser, `--host`, `--list-hosts`, `--load`, `--save`, `--transcript`, command recall, restart/change-host controls, save/reload shortcuts, native save-slot browse/filter/sort/inspect/save/load/delete/export controls, structured region rendering, clickable command hint buttons, and snapshot panel/history/status rendering
