...
pub struct MuddleMacroquadRunOptions {
    ...
    pub visual_smoke_panel_contains: Option<String>,
    pub visual_smoke_command_contains: Option<String>,
...
impl Default for MuddleMacroquadRunOptions {
    fn default() -> Self {
        Self {
            ...
            visual_smoke_panel_contains: None,
            visual_smoke_command_contains: None,
    ...
    if options.visual_smoke {
        println!(
            ...
            macroquad_visual_smoke(
                ...
                options.visual_smoke_panel_contains.as_deref(),
                options.visual_smoke_command_contains.as_deref(),
...
pub fn parse_macroquad_run_options(
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--visual-smoke-min-nodes" => {
                let value = args
                    .next()
                    .ok_or_else(|| "`--visual-smoke-min-nodes` requires a count.".to_string())?;
                options.visual_smoke_min_nodes = Some(parse_visual_smoke_count(
                    "--visual-smoke-min-nodes",
                    &value,
                )?);
            }
            "--visual-smoke-min-frames" => {
                let value = args
                    .next()
                    .ok_or_else(|| "`--visual-smoke-min-frames` requires a count.".to_string())?;
                options.visual_smoke_min_frames = Some(parse_visual_smoke_count(
                    "--visual-smoke-min-frames",
                    &value,
                )?);
            }
            ...
            "--visual-smoke-response-contains" => {
                options.visual_smoke_response_contains = Some(args.next().ok_or_else(|| {
            ...
            "--visual-smoke-panel-contains" => {
                options.visual_smoke_panel_contains =
                    Some(args.next().ok_or_else(|| {
                        "`--visual-smoke-panel-contains` requires text.".to_string()
                    })?);
            }
            "--visual-smoke-command-contains" => {
                options.visual_smoke_command_contains = Some(args.next().ok_or_else(|| {
                    "`--visual-smoke-command-contains` requires text.".to_string()
                })?);
            }
            ...
            _ => {
                ...
                } else if let Some(value) = arg.strip_prefix("--visual-smoke-min-nodes=") {
                    options.visual_smoke_min_nodes =
                        Some(parse_visual_smoke_count("--visual-smoke-min-nodes", value)?);
                } else if let Some(value) = arg.strip_prefix("--visual-smoke-min-frames=") {
                    options.visual_smoke_min_frames = Some(parse_visual_smoke_count(
                        "--visual-smoke-min-frames",
                        value,
                    )?);
                ...
                } else if let Some(value) = arg.strip_prefix("--visual-smoke-response-contains=") {
                    if value.is_empty() {
                ...
                } else if let Some(value) = arg.strip_prefix("--visual-smoke-panel-contains=") {
                    if value.is_empty() {
                        return Err("`--visual-smoke-panel-contains` requires text.".to_string());
                    }
                    options.visual_smoke_panel_contains = Some(value.to_string());
                } else if let Some(value) = arg.strip_prefix("--visual-smoke-command-contains=") {
                    if value.is_empty() {
                        return Err("`--visual-smoke-command-contains` requires text.".to_string());
                    }
                    options.visual_smoke_command_contains = Some(value.to_string());
                } else {
...
fn parse_visual_smoke_count(flag: &str, value: &str) -> Result<usize, String> {
    if value.is_empty() {
        return Err(format!("`{flag}` requires a count."));
    }
    value
        .parse::<usize>()
        .map_err(|_| format!("`{flag}` requires a non-negative integer count."))
}

pub fn macroquad_usage() -> &'static str {
    "Usage: muddle-macroquad [--host <name>] [--load <path>] [--save <path>] [--transcript <path>] [--import <path>] [--export <path>] [--list-hosts] [--visual-smoke] [--require-visuals] [--require-visual-frames] [--require-visual-animation] [--visual-smoke-room <room-id>] [--visual-smoke-min-nodes <count>] [--visual-smoke-min-frames <count>] [--visual-smoke-min-turns <count>] [--visual-smoke-response-contains <text>] [--visual-smoke-visual-text-contains <text>] [--visual-smoke-panel-contains <text>] [--visual-smoke-command-contains <text>] [--visual-smoke-command <command>] [--help]"
}
...
pub fn macroquad_visual_smoke(
    ...
    let mut panel_mismatch_hosts = Vec::new();
    let mut command_mismatch_hosts = Vec::new();
    for registration in registrations {
        let mut host = (registration.create)();
        ...
        let last_response = session
            ...
            .unwrap_or("Visual smoke snapshot.");
        let snapshot =
            session.client_snapshot(host.as_ref(), registration.client_info(), last_response);
        ...
        let status = if layout.visual_nodes.is_empty() {
            ...
            low_node_hosts.push(format!(
                ...
                registration.name,
                min_nodes.unwrap_or_default(),
                layout.visual_nodes.len()
            ));
            "too few visual nodes"
        } else if min_frames.is_some_and(|minimum| frame_count < minimum) {
            low_frame_hosts.push(format!(
                "{} expected at least {} got {}",
                registration.name,
                min_frames.unwrap_or_default(),
                frame_count
            ));
            ...
            "too few turns"
        } else if response_contains
            .is_some_and(|expected| !snapshot.last_response.contains(expected))
        {
            response_mismatch_hosts.push(format!(
        ...
        } else if panel_contains.is_some_and(|expected| !panels_contain_text(&layout, expected)) {
            panel_mismatch_hosts.push(format!(
                "{} expected panel text containing {:?}",
                registration.name,
                panel_contains.unwrap_or_default()
            ));
            "unexpected panel text"
        } else if command_contains.is_some_and(|expected| !commands_contain_text(&layout, expected))
        {
            command_mismatch_hosts.push(format!(
                "{} expected command text containing {:?}",
                registration.name,
                command_contains.unwrap_or_default()
            ));
    ...
    if !low_node_hosts.is_empty() {
        return Err(format!(
            "visual smoke failed: hosts below visual node minimum: {}",
            low_node_hosts.join(", ")
        ));
    }
    if !low_frame_hosts.is_empty() {
        return Err(format!(
    ...
    if !response_mismatch_hosts.is_empty() {
        return Err(format!(
            "visual smoke failed: hosts with unexpected responses: {}",
    ...
    if !panel_mismatch_hosts.is_empty() {
        return Err(format!(
            "visual smoke failed: hosts with unexpected panel text: {}",
            panel_mismatch_hosts.join(", ")
        ));
    }
    if !command_mismatch_hosts.is_empty() {
        return Err(format!(
            "visual smoke failed: hosts with unexpected command text: {}",
            command_mismatch_hosts.join(", ")
        ));
    }
...
fn panels_contain_text(layout: &MuddleMacroquadPlayLayout, expected: &str) -> bool {
    layout.panels.iter().any(|panel| {
        panel.label.contains(expected) || panel.lines.iter().any(|line| line.contains(expected))
    })
}

fn commands_contain_text(layout: &MuddleMacroquadPlayLayout, expected: &str) -> bool {
    layout
        .commands
        .iter()
        .any(|command| command.label.contains(expected) || command.command.contains(expected))
}
...
mod tests {
    ...
    fn macroquad_args_parse_host_and_list() {
        let options = parse_macroquad_run_options([
            ...
            "--visual-smoke-panel-contains=Scene room".to_string(),
            "--visual-smoke-command-contains=advance".to_string(),
    ...
    fn macroquad_visual_smoke_can_require_minimum_counts() {
        ...
        assert!(low_nodes.contains("expected at least 3 got 2"));

        let low_frames = macroquad_visual_smoke(
            &[MuddleClientHostRegistration {
                name: "visual-host",
                category: "Tests",
                description: "Visual smoke test host.",
    ...
    #[test]
    fn macroquad_visual_smoke_can_require_response_text() {
        let error = macroquad_visual_smoke(
            &[MuddleClientHostRegistration {
                name: "visual-host",
                category: "Tests",
    ...
    #[test]
    fn macroquad_visual_smoke_can_require_panel_text() {
        let error = macroquad_visual_smoke(
            &[MuddleClientHostRegistration {
                name: "visual-host",
                category: "Tests",
        ...
        .expect_err("visual smoke fails on unexpected panel text");
    ...
    #[test]
    fn macroquad_visual_smoke_can_require_command_text() {
        let error = macroquad_visual_smoke(
            &[MuddleClientHostRegistration {
                name: "visual-host",
                category: "Tests",
        ...
        .expect_err("visual smoke fails on unexpected command text");
    ...
    impl MuddleHost for VisualSmokeTestHost {
        ...
        fn resource_panel(&self) -> Vec<muddle_core::MuddleResource> {
            vec![muddle_core::MuddleResource {
                label: "Panel".to_string(),
                value: "Scene room".to_string(),
            }]
        }

        fn command_panel(&self, _current_room: &str) -> Vec<muddle_core::MuddleCommandHint> {
            vec![muddle_core::MuddleCommandHint {
                command: "advance".to_string(),
                description: "Advance the visual smoke host.".to_string(),
            }]
        }
